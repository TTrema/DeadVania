...
class Controls_Handler:
    ...
    def render(self, surface):
        ...
        if self.curr_block == self.save_file["current_profile"]:
            self.draw_text(surface, "*", 20, pygame.Color((0, 0, 0)), 20, 20)
    ...
    def navigate_menu(self, actions):
        ...
        if actions["Right"]:
            self.curr_block = (self.curr_block + 1) % len(self.save_file["controls"])
        # Handle Selection
        if actions["Jump"] or actions["Start"]:
    ...
    def display_controls(self, surface, controls):
        ...
        for control in controls:
            self.draw_text(surface, control + " - " + pygame.key.name(controls[control]), 20, pygame.Color((0, 0, 0)), 480 / 2, 270 / 4 + i)
            i += 30
